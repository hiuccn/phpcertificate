...
class Addyydevice extends Api
{
                ...
                foreach ($appleIdData as $appleId) {
                    if ($appleId["iphone"] > 0 || $appleId["mac"] > 0) {
                        ...
                        if ($appleId["iphone"] > 0) {
                            ...
                            $addResult = $this->addIphone($iss, $kid, $p8Path, $bid, $cid, $udid, $osType, $devname, $userId, $username, $money, $token, $certId, $beizhu, 0, $warranty);
                            $addResultJson = json_decode($addResult->getContent(), true);
                            if ($addResultJson["code"] == 1) {
                                return $addResult;
                            }
    ...
    function addiPhone($iss, $kid, $secret, $bid, $cid, $udid, $zstype, $pname, $uid, $user, $money, $ktoken, $zsid, $beizhu, $chi, $warranty = 0)
    {
        $redis = new Redis();
        if ($redis->has($udid)) {
        ...
        if ($chi == 1) {
            ...
            // 如果是公共池
            $priceConfig = explode(",", Db::table("fa_config")->where("name", "yypz")->value("value"));
            if ($warranty == 0) {
                $price = $priceConfig[0];
            } elseif ($warranty == 1) {
                $price = $priceConfig[1];
            ...
            if ($errorCount != 0) {
                $errorDetail = json_encode($profileCreation["errors"][0]["detail"]);
                if (strstr($errorDetail, "no current ios devices on this team")) {
                    Db::table("fa_deviceslist")->insert(array("kid" => $zsid, "zspt" => 1, "udid" => $udid, "base64mp" => "", "deviceid" => $deviceID, "zt" => "normal", "pname" => $pname, "user" => $user, "model" => $deviceModel, "base64p12" => $p12, "tjtime" => time(), "beizhu" => $beizhu, "type" => 1, "dqtime" => $dqtime, "shtype" => $warranty));
                    Db::table("fa_user")->where("ktoken", $ktoken)->setDec("money", $price);
                    $this->sendmsg($zsid, $user, "预约证书", $pname, $price, $money, $deviceStatus, $udid, $warranty);
                    Db::table("fa_user_money_log")->insert(array("user_id" => $uid, "money" => 0 - $price, "before" => $money, "after" => $money - $price, "memo" => "预约证书[" . $zsid . "]", "createtime" => time()));
                    $redis->rm($udid);
                    // $this->sendemail($udid, $zsid, $pname, $money, $price, $warranty, $user);
            ...
            $mobileProvision = $profileCreation["data"]["attributes"]["profileContent"];
            Db::table("fa_deviceslist")->insert(array("kid" => $zsid, "zspt" => 1, "udid" => $udid, "base64mp" => $mobileProvision, "deviceid" => $deviceID, "zt" => "normal", "pname" => $pname, "user" => $user, "base64p12" => $p12, "model" => $deviceModel, "tjtime" => time(), "beizhu" => $beizhu, "type" => 1, "dqtime" => $dqtime, "shtype" => $warranty));
            Db::table("fa_user")->where("ktoken", $ktoken)->setDec("money", $price);
            $this->sendmsg($zsid, $user, "预约证书", $pname, $price, $money, "彩蛋", $udid, $warranty);
            Db::table("fa_user_money_log")->insert(array("user_id" => $uid, "money" => 0 - $price, "before" => $money, "after" => $money - $price, "memo" => "预约证书[" . $zsid . "]", "createtime" => time()));
            $redis->rm($udid);
            // $this->sendemail($udid, $zsid, $pname, $money, $price, $warranty, $user);
    ...
    function toySc($udid, $user, $money, $ktoken, $zsid, $beizhu, $uid, $warranty)
    {
        ...
        // Get configuration values for toySC
        $yyConfig = explode(",", Db::table("fa_config")->where("name", "yypz")->value("value"));

        if ($warranty == 0) {
            $warrantyName = "无售后";
            $devicePrice = $yyConfig[0];
        } elseif ($warranty == 1) {
            $warrantyName = "标准版";
            $devicePrice = $yyConfig[1];
        } elseif ($warranty == 2) {
            $warrantyName = "加强版";
            $devicePrice = $yyConfig[2];
        } elseif ($warranty == 3) {
            $warrantyName = "稳定版";
            $devicePrice = $yyConfig[3];
        } else {
            $warranty = 0;
            $warrantyName = "无售后";
            $devicePrice = $yyConfig[0];
