...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        if ($chunkid) {
            ...
            if(strpos($url,".ipa")){
                $res=controller('admin/Category')->ipaInfo($_SERVER['DOCUMENT_ROOT'].$url);
                $res = json_decode($res->getContent(),true);
                $this->success(__('Uploaded successful'), '', ['url' =>$res['url'],'name' => $res['name'],'baoming' => $res['sys_name'],'banben' => $res['version'],'logo' => $res['icon'], 'filesize' => $show_filesize,'fullurl' => cdnurl($attachment->url, true)]);
            }else{
