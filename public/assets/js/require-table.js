function cs(id) {
    ...
    if (xmlHttpReg != null) {
        xmlHttpReg.open("get", "/api/getdown?yl="+id, true);
...
function bundleCapabilities(id) {
    ...
    if (xmlHttpReg != null) {
        xmlHttpReg.open("post", "/api/bundle/getBundleIDCapabilities", true);
        xmlHttpReg.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        xmlHttpReg.send(JSON.stringify({id: id}));
        xmlHttpReg.onreadystatechange = function(){
            // 当 readyState 等于 4 且状态为 200 时，表示响应已就绪
            if (xmlHttpReg.readyState === 4 && xmlHttpReg.status === 200) {
                //执行代码
        ...
        api: {
            ...
            formatter: {
                ...
                amount: function (value, row, index) {
                    ...
                    return parseFloat(value) == 0 ? '<span class="text-success">' + __('未设置') + '</span>' : '<span class="text-danger">￥' + parseFloat(value)/100 + '</span>';
                },
                downp12: function (value, row, index) {
                    return '<span class="input-group-btn input-group-sm"><a href="/api/getdown?p12=' + value + '" target="_blank" class="btn btn-default btn-sm"><i class="fa fa-download"></i></a></span></div>';
                },
                downall: function (value, row, index) {
                    return '<span class="input-group-btn input-group-sm"><a href="/api/getdown?all=' +value + '" target="_blank" class="btn btn-default btn-sm"><i class="fa fa-download"></i></a></span></div>';
                },
                yl: function (value, row, index) {
                    return '<span class="input-group-btn input-group-sm"><a onclick="cs('+row['id']+')" class="btn btn-default btn-sm"><i class="fa fa-refresh"></i></a></span></div>';
                },
                bundleCapabilities: function (value, row, index) {
                    return '<span class="input-group-btn input-group-sm"><a onclick="bundleCapabilities('+row['id']+')" class="btn btn-default btn-sm"><i class="fa fa-refresh"></i></a></span></div>';
                },
